...
export function useExtensionMessages(
  ...
  useEffect(() => {
    ...
    const handler = (msg: Record<string, unknown>) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        if (layoutReadyRef.current && isEditDirty?.()) {
          console.log('[Standalone] Skipping external layout update — editor has unsaved changes')
          return
        }
        ...
        console.log(`[Standalone] Received ${characters.length} pre-colored character sprites`)
        setCharacterTemplates(characters)
      } else if (msg.type === 'floorTilesLoaded') {
        const sprites = msg.sprites as string[][][]
        console.log(`[Standalone] Received ${sprites.length} floor tile patterns`)
        setFloorSprites(sprites)
      } else if (msg.type === 'wallTilesLoaded') {
        const sprites = msg.sprites as string[][][]
        console.log(`[Standalone] Received ${sprites.length} wall tile sprites`)
        ...
        try {
          ...
          console.log(`[Standalone] Loaded ${catalog.length} furniture assets`)
          // Build dynamic catalog immediately so getCatalogEntry() works when layoutLoaded arrives next
          buildDynamicCatalog({ catalog, sprites })
          setLoadedAssets({ catalog, sprites })
        } catch (err) {
          console.error(`[Standalone] Error processing furnitureAssetsLoaded:`, err)
        }
      }
    }
    ...
    const eventTypes = [
      ...
      'subagentToolPermission', 'subagentToolStart', 'subagentToolDone', 'subagentClear',
      'characterSpritesLoaded', 'floorTilesLoaded', 'wallTilesLoaded',
      'settingsLoaded', 'furnitureAssetsLoaded',
    ]
    const unsubscribes = eventTypes.map((type) => eventBus.on(type, handler))
